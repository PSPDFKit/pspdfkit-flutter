...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    if ([@"frameworkVersion" isEqualToString:call.method]) {
        ...
        result([self importXFDFFromPath:path]);
    } else if ([@"exportXfdf" isEqualToString:call.method]) {
        NSString *path = call.arguments[@"xfdfPath"];
        result([self exportXFDFToPath:path]);
...
- (id)exportXFDFToPath:(NSString *)path {
    // Always overwrite the XFDF file we export to.
    NSURL *fileURL = [self writableFileURLWithPath:path override:YES copyIfNeeded:NO];
