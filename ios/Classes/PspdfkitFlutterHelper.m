...
+ (void)processMethodCall:(FlutterMethodCall *)call
                   result:(FlutterResult)result
        forViewController:(PSPDFViewController *)pdfViewController {
    if ([@"save" isEqualToString:call.method]) {
        ...
        if (!document || !document.isValid) {
            result([FlutterError errorWithCode:@""
                                       message:@"PDF document not found or is invalid."
                                       details:nil]);
            return;
        }
        [document
            ...
            completionHandler:^(NSError *_Nullable error,
                                NSArray<__kindof PSPDFAnnotation *> *_Nonnull savedAnnotations) {
              if (error == nil) {
                  result(@(YES));
              } else {
        ...
        result([PspdfkitFlutterHelper setFormFieldValue:value
                         forFieldWithFullyQualifiedName:fullyQualifiedName
                                      forViewController:pdfViewController]);
    } else if ([@"getFormFieldValue" isEqualToString:call.method]) {
        NSString *fullyQualifiedName = call.arguments[@"fullyQualifiedName"];
        result([PspdfkitFlutterHelper
            getFormFieldValueForFieldWithFullyQualifiedName:fullyQualifiedName
                                          forViewController:pdfViewController]);
        ...
        if (annotationsJson.length == 0) {
            result([FlutterError errorWithCode:@""
                                       message:@"annotationsJson may not be nil or empty."
                                       details:nil]);
            return;
        }
        PSPDFDocument *document = pdfViewController.document;
        ...
        PSPDFDataContainerProvider *jsonContainer = [[PSPDFDataContainerProvider alloc]
            initWithData:[annotationsJson dataUsingEncoding:NSUTF8StringEncoding]];
        NSError *error;
        BOOL success =
            [document applyInstantJSONFromDataProvider:jsonContainer
                                    toDocumentProvider:document.documentProviders.firstObject
                                               lenient:NO
                                                 error:&error];
        if (!success) {
            result([FlutterError errorWithCode:@""
                                       message:@"Error while importing document Instant JSON."
                                       details:nil]);
        } else {
        ...
        NSError *error;
        NSData *data =
            [document generateInstantJSONFromDocumentProvider:document.documentProviders.firstObject
                                                        error:&error];
        NSString *annotationsJson = [[NSString alloc] initWithData:data
                                                          encoding:NSUTF8StringEncoding];
        if (annotationsJson == nil) {
            result([FlutterError errorWithCode:@""
                                       message:@"Error while exporting document Instant JSON."
        ...
        result([PspdfkitFlutterHelper addAnnotation:jsonAnnotation
                                  forViewController:pdfViewController]);
    } else if ([@"removeAnnotation" isEqualToString:call.method]) {
        id jsonAnnotation = call.arguments[@"jsonAnnotation"];
        result([PspdfkitFlutterHelper removeAnnotation:jsonAnnotation
                                     forViewController:pdfViewController]);
        ...
        result([PspdfkitFlutterHelper getAnnotationsForPageIndex:pageIndex
                                                         andType:typeString
                                               forViewController:pdfViewController]);
...
+ (BOOL)isImageDocument:(NSString *)path {
    ...
    return [fileExtension isEqualToString:@"png"] || [fileExtension isEqualToString:@"jpeg"] ||
           [fileExtension isEqualToString:@"jpg"] || [fileExtension isEqualToString:@"tiff"] ||
...
+ (NSURL *)fileURLWithPath:(NSString *)path {
    if (path) {
        ...
        if (![path isAbsolutePath]) {
            path = [[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"www"]
...
+ (NSURL *)writableFileURLWithPath:(NSString *)path
                          override:(BOOL)override
                      copyIfNeeded:(BOOL)copyIfNeeded {
    ...
    // If we don't have a writable file already, we move the provided file to the ~/Documents
    // folder.
    if (![fileManager fileExistsAtPath:(NSString *)writableFileURL.path]) {
        ...
        if (![fileManager
                      createDirectoryAtPath:writableFileURL.path.stringByDeletingLastPathComponent
        ...
        if (copyIfNeeded && [fileManager fileExistsAtPath:(NSString *)fileURL.path]) {
            if (![fileManager copyItemAtURL:fileURL toURL:writableFileURL error:&copyError]) {
                NSLog(@"Failed to copy item at URL '%@' with error: %@", path,
...
#pragma mark - Document Info Customization
+ (void)setDocumentInfoOptions:(nullable NSArray<NSString *> *)items
             forViewController:(PSPDFViewController *)pdfViewController {
    if ((id)items == NSNull.null || !items || items.count == 0) {
        return;
    }
    NSMutableArray *documentInfoItems = [NSMutableArray array];
    for (NSString *infoItem in items) {
        if ([infoItem isEqualToString:@"outline"]) {
          [documentInfoItems addObject: PSPDFDocumentInfoOptionOutline];
        } else if ([infoItem isEqualToString:@"annotations"]) {
          [documentInfoItems addObject: PSPDFDocumentInfoOptionAnnotations];
        } else if ([infoItem isEqualToString:@"embeddedFiles"]) {
          [documentInfoItems addObject: PSPDFDocumentInfoOptionEmbeddedFiles];
        } else if ([infoItem isEqualToString:@"bookmarks"]) {
          [documentInfoItems addObject: PSPDFDocumentInfoOptionBookmarks];
        } else if ([infoItem isEqualToString:@"documentInfo"]) {
          [documentInfoItems addObject: PSPDFDocumentInfoOptionDocumentInfo];
        } else if ([infoItem isEqualToString:@"security"]) {
          [documentInfoItems addObject: PSPDFDocumentInfoOptionSecurity];
        }
    }
    pdfViewController.documentInfoCoordinator.availableControllerOptions = documentInfoItems;
}
...
+ (void)setLeftBarButtonItems:(nullable NSArray<NSString *> *)items
    ...
    for (NSString *barButtonItemString in items) {
        UIBarButtonItem *barButtonItem = [self barButtonItemFromString:barButtonItemString
                                                     forViewController:pdfViewController];
        if (barButtonItem &&
            ![pdfViewController.navigationItem.rightBarButtonItems containsObject:barButtonItem]) {
...
+ (void)setRightBarButtonItems:(nullable NSArray<NSString *> *)items
    ...
    for (NSString *barButtonItemString in items) {
        UIBarButtonItem *barButtonItem =
            [PspdfkitFlutterHelper barButtonItemFromString:barButtonItemString
                                         forViewController:pdfViewController];
        if (barButtonItem &&
            ![pdfViewController.navigationItem.leftBarButtonItems containsObject:barButtonItem]) {
...
+ (UIBarButtonItem *)barButtonItemFromString:(NSString *)barButtonItem
                           forViewController:(PSPDFViewController *)pdfViewController {
...
+ (id)setFormFieldValue:(NSString *)value
    forFieldWithFullyQualifiedName:(NSString *)fullyQualifiedName
                 forViewController:(PSPDFViewController *)pdfViewController {
    PSPDFDocument *document = pdfViewController.document;

    if (!document || !document.isValid) {
        FlutterError *error = [FlutterError errorWithCode:@""
                                                  message:@"PDF document not found or is invalid."
                                                  details:nil];
        return error;
    }
    ...
    if (fullyQualifiedName == nil || fullyQualifiedName.length == 0) {
        FlutterError *error =
            [FlutterError errorWithCode:@""
                                message:@"Fully qualified name may not be nil or empty."
                                details:nil];
        return error;
    }
    ...
    for (PSPDFFormElement *formElement in document.formParser.forms) {
        if ([formElement.fullyQualifiedFieldName isEqualToString:fullyQualifiedName]) {
            if ([formElement isKindOfClass:PSPDFButtonFormElement.class]) {
                ...
                ((PSPDFChoiceFormElement *)formElement).selectedIndices =
                    [NSIndexSet indexSetWithIndex:value.integerValue];
                success = YES;
                ...
                FlutterError *error =
                    [FlutterError errorWithCode:@""
                                        message:@"Signature form elements are not supported."
                                        details:nil];
                return error;
            } else {
                return @(NO);
        ...
        FlutterError *error = [FlutterError
            errorWithCode:@""
                  message:[NSString stringWithFormat:
                                        @"Error while searching for a form element with name %@.",
                                        fullyQualifiedName]
                  details:nil];
        return error;
    }
...
+ (id)processAnnotationsOfType:(NSString *)type
            withProcessingMode:(NSString *)processingMode
            andDestinationPath:(NSString *)destinationPath
             forViewController:(PSPDFViewController *)pdfViewController {
    PSPDFAnnotationChange change =
        [PspdfkitFlutterConverter annotationChangeFromString:processingMode];
    NSURL *processedDocumentURL = [PspdfkitFlutterHelper writableFileURLWithPath:destinationPath
                                                                        override:YES
                                                                    copyIfNeeded:NO];
    PSPDFAnnotationType annotationType = [PspdfkitFlutterConverter annotationTypeFromString:type];

    if (!processedDocumentURL) {
        return [FlutterError errorWithCode:@""
                                   message:@"Could not create a new PDF file at the given path."
                                   details:nil];
    }
    ...
    PSPDFProcessorConfiguration *configuration =
        [[PSPDFProcessorConfiguration alloc] initWithDocument:document];

    // Modify annotations.
    [configuration modifyAnnotationsOfTypes:annotationType change:change];

    // Create the PDF processor and write the processed file.
    PSPDFProcessor *processor = [[PSPDFProcessor alloc] initWithConfiguration:configuration
                                                              securityOptions:nil];
    ...
    if (error) {
        return [FlutterError errorWithCode:@""
                                   message:@"Error writing to PDF file."
                                   details:error.localizedDescription];
    }
...
+ (id)addAnnotation:(id)jsonAnnotation forViewController:(PSPDFViewController *)pdfViewController {
    ...
    PSPDFAnnotation *annotation = [PSPDFAnnotation annotationFromInstantJSON:data
...
+ (id)removeAnnotation:(id)jsonAnnotation
    ...
    if ([jsonAnnotation isKindOfClass:NSString.class]) {
        ...
        NSDictionary *jsonDict = [NSJSONSerialization JSONObjectWithData:jsonData
                                                                 options:NSJSONReadingAllowFragments
                                                                   error:nil];
        if (jsonDict) {
            annotationUUID = jsonDict[@"uuid"];
        }
    } else if ([jsonAnnotation isKindOfClass:NSDictionary.class]) {
        if (jsonAnnotation) {
            annotationUUID = jsonAnnotation[@"uuid"];
        }
    }
    ...
    BOOL success = NO;
    NSArray<PSPDFAnnotation *> *allAnnotations =
        [[document allAnnotationsOfType:PSPDFAnnotationTypeAll].allValues
...
+ (id)getAnnotationsForPageIndex:(PSPDFPageIndex)pageIndex
                         andType:(NSString *)typeString
               forViewController:(PSPDFViewController *)pdfViewController {
    PSPDFDocument *document = pdfViewController.document;
    if (!document || !document.isValid) {
        return [FlutterError errorWithCode:@""
    ...
    NSArray<PSPDFAnnotation *> *annotations = [document annotationsForPageAtIndex:pageIndex
                                                                             type:type];
    NSArray<NSDictionary *> *annotationsJSON =
        [PspdfkitFlutterConverter instantJSONFromAnnotations:annotations];
...
+ (id)getAllUnsavedAnnotationsForViewController:(PSPDFViewController *)pdfViewController {
    ...
    NSDictionary *annotationsJSON = [NSJSONSerialization JSONObjectWithData:data
                                                                    options:kNilOptions
                                                                      error:NULL];
...
+ (id)importXFDFFromPath:(NSString *)path
       forViewController:(PSPDFViewController *)pdfViewController {
    ...
    if (![NSFileManager.defaultManager fileExistsAtPath:(NSString *)fileURL.path]) {
        return [FlutterError errorWithCode:@""
                                   message:@"The XFDF file does not exist."
                                   details:nil];
    }
    ...
    PSPDFXFDFParser *parser =
        [[PSPDFXFDFParser alloc] initWithDataProvider:dataProvider
    ...
    if (error) {
        return [FlutterError errorWithCode:@""
                                   message:@"Error while parsing XFDF file."
                                   details:error.localizedDescription];
    }
...
+ (id)exportXFDFToPath:(NSString *)path forViewController:(PSPDFViewController *)pdfViewController {
    ...
    NSURL *fileURL = [PspdfkitFlutterHelper writableFileURLWithPath:path
                                                           override:YES
                                                       copyIfNeeded:NO];

    if (!fileURL) {
        return [FlutterError errorWithCode:@""
                                   message:@"Could not create a new XFDF file at the given path."
                                   details:nil];
    }
    ...
    for (NSArray *pageAnnotations in [document allAnnotationsOfType:PSPDFAnnotationTypeAll]
             .allValues) {
    ...
    NSError *error;
    PSPDFFileDataSink *dataSink = [[PSPDFFileDataSink alloc] initWithFileURL:fileURL
                                                                     options:PSPDFDataSinkOptionNone
                                                                       error:&error];
    if (error) {
        return [FlutterError errorWithCode:@""
                                   message:@"Error while exporting XFDF file."
                                   details:error.localizedDescription];
    }

    [[PSPDFXFDFWriter new] writeAnnotations:annotations
                                 toDataSink:dataSink
                           documentProvider:document.documentProviders[0]
                                      error:&error];
    if (error) {
