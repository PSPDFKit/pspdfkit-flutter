...
class InstantCollaborationExample extends StatefulWidget {
  const InstantCollaborationExample({Key? key}) : super(key: key);

  @override
  State<InstantCollaborationExample> createState() =>
      _InstantCollaborationExampleState();
}

class _InstantCollaborationExampleState
    extends State<InstantCollaborationExample> {
  ...
  void _loadDocument(BuildContext context, String? url) {
    if (url != null) {
      ...
      _getDocument(url).then((doc) async {
        Navigator.of(context).pop();
        await Nutrient.presentInstant(doc.serverUrl, doc.jwt,
            PdfConfiguration(enableInstantComments: enableComments));

        await Nutrient.setDelayForSyncingLocalChanges(delayTime);
        await Nutrient.setListenToServerChanges(enableListenToServerChanges);
