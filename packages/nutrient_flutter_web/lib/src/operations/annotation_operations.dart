...
///
/// This class wraps the raw [nutrient_web.Instance] annotation APIs and provides
...
class NutrientAnnotationOperations {
  ...
  ///
  /// The optional [attachment] is a JSON string with keys `id`, `binary`
  /// (base64-encoded), and `contentType` — as produced by
  /// `AnnotationAttachment.toJson()`. When provided, the binary is uploaded via
  /// `instance.createAttachment()` and the returned attachment ID is set as
  /// `imageAttachmentId` on the annotation map before creation, so image/stamp
  /// annotations render their image.
  ///
  ...
  Future<void> addAnnotation(
    ...
    // If a binary attachment is provided, upload it and set imageAttachmentId.
    if (attachment != null) {
      final attachmentId = await _uploadAttachment(attachment);
      if (attachmentId != null) {
        jsonAnnotation = Map<String, dynamic>.from(jsonAnnotation)
    ...
    // The generated `create` takes a union extension type; cross-cast
    // through JSAny to satisfy the static type-check. The runtime
    // accepts plain Web SDK annotation JS objects either way.
    await (_instance as JSObject)
        .callMethod<JSPromise>('create'.toJS, webAnnotation)
        .toDart;
  }

  /// Decodes [attachmentJson] (a JSON string with `binary` in base64 and a
  /// `contentType`), builds a [web.Blob], calls `instance.createAttachment()`,
  /// and returns the resulting attachment ID string.
  ///
  /// Returns `null` if the JSON is malformed or the binary field is absent.
  Future<String?> _uploadAttachment(String attachmentJson) async {
    Map<String, dynamic> map;
    try {
      map = jsonDecode(attachmentJson) as Map<String, dynamic>;
    } catch (_) {
      return null;
    }

    final base64Binary = map['binary'] as String?;
    final contentType =
        (map['contentType'] as String?) ?? 'application/octet-stream';

    if (base64Binary == null || base64Binary.isEmpty) return null;

    final Uint8List bytes = base64Decode(base64Binary);
    final blob = web.Blob(
      [bytes.toJS].toJS,
      web.BlobPropertyBag(type: contentType),
    );

    final attachmentId = await _instance.createAttachment(blob).toDart;
    return attachmentId.toDart;
  }
      ...
      if ((itemId == annotationId && itemPage == pageIndex) ||
        ...
        await (_instance as JSObject)
            .callMethod<JSPromise>(
                'delete'.toJS, (itemId ?? itemName ?? '').toJS)
            .toDart;
        return;
      }
    }
  ...
  Future<void> updateAnnotationProperties(
    ...
    // `AnnotationProperties.toJson()` keys the id as `annotationId`; the raw
    // Web SDK shape uses `id` / `name`. Accept all three so this works whether
    // it's fed a model map or a raw annotation map.
    final annotationId =
        properties['annotationId'] ?? properties['id'] ?? properties['name'];
    ...
    for (final entry in properties.entries) {
      ...
      // Skip identity fields. `annotationId` is the model's id key (see
      // updateAnnotationProperties' lookup above); the raw SDK shape uses
      // `id` / `name`.
      if (key == 'annotationId' ||
          key == 'id' ||
          key == 'pageIndex' ||
      ...
      // Handle color properties. `AnnotationProperties.toJson()` emits colours
      // as ARGB ints; raw SDK maps use `{r,g,b}`; older callers may pass a hex
      // string. `parseColorFromJson` handles Map/String, so accept int here
      // directly and let it pass through the others.
      if (webKey == 'strokeColor' || webKey == 'fillColor') {
        ...
        updated = ImmutableRecord.set(
          updated,
          webKey,
          WebColorUtils.colorIntToWebColor(argb) as JSAny,
        );
        continue;
      }
      ...
      // Handle customData. The Web SDK stores it under `customData` as an
      // object. Accept both the raw `customData` object and the model's
      // `customDataJson` (a JSON string), decoding the latter so custom data
      // actually round-trips through saveAnnotationProperties.
      if (key == 'customData' && value is Map) {
        updated = ImmutableRecord.set(updated, 'customData', value.jsify());
        continue;
      }
      if (key == 'customDataJson') {
        Object? decoded;
        if (value is String) {
          try {
        ...
        if (decoded != null) {
          updated = ImmutableRecord.set(updated, 'customData', decoded.jsify());
        }
        continue;
      }
    ...
    await (_instance as JSObject)
        .callMethod<JSPromise>('update'.toJS, updated)
        .toDart;
  }

  /// Searches for annotations matching a query string.
  Future<JSAny?> searchAnnotations(String query) async {
    return (_instance as JSObject)
        .callMethod<JSPromise>('search'.toJS, query.toJS, null)
        .toDart;
  }
  ...
  Future<JSAny?> _getRawAnnotations(int pageIndex) async {
    // The generator types `Instance.getAnnotations` as
    // `JSPromise<JSFunction>` (TS source returns `Iterable<Annotation>`,
    // which the generator currently maps to JSFunction). At runtime the
    // SDK resolves to an Immutable.List — a plain JS object. Awaiting
    // the typed promise would fail the implicit `as JSFunction` cast,
    // so route through `callMethod` to keep the resolved value as JSAny.
    return (_instance as JSObject)
        .callMethod<JSPromise>('getAnnotations'.toJS, pageIndex.toJS)
        .toDart;
  }
