...
class NutrientFormOperations {
  ...
  Future<List<Map<String, dynamic>>> getFormFields() async {
    // The generator types `Instance.getFormFields` as
    // `JSPromise<JSFunction>` (TS returns Iterable<FormField>). The
    // SDK resolves to an Immutable.List — a plain JS object — so
    // awaiting the typed promise would fail the implicit
    // `as JSFunction` cast. Route through `callMethod` to keep the
    // resolved value as JSAny.
    final JSAny? result = await (_instance as JSObject)
        .callMethod<JSPromise>('getFormFields'.toJS)
        .toDart;
