...
#
# REGENERATING AGAINST A KOTLIN 2.3+ SDK (e.g. the nightly / SNAPSHOT):
#   Released jnigen ships kotlinx-metadata-jvm:0.9.0, which only reads Kotlin
#   metadata up to 2.1 and fails on the nightly with:
#     "Provided Metadata instance has version 2.3.0, while maximum supported
#      version is 2.1.0".
#   Until jnigen ships Kotlin 2.3 support (dart-lang/native #3219 / #3269 /
#   PR #3297), regenerate with a patched jnigen:
#     1. Clone dart-lang/native (jnigen main has PR #3297 which migrates the
#        ApiSummarizer to org.jetbrains.kotlin:kotlin-metadata-jvm).
#     2. In pkgs/jnigen/java/build.gradle.kts bump kotlin-metadata-jvm to 2.3.0.
#     3. Guard pkgs/jnigen/lib/src/elements/elements.dart KotlinType.toDocComment
#        against out-of-range type-parameter indices (id < list.length).
#     4. Point the jnigen dev_dependency in pubspec.yaml at that local checkout.
#   The post-processor (scripts/fix_bindings.sh) cleans up the remaining
#   jnigen 0.17 codegen quirks (nullable setter params, BasePdfUiBuilder).
