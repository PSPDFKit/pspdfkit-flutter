...
abstract class Annotation {
  ...
  /// Parses a `measurementScale` value that may arrive as an already-typed
  /// [MeasurementScale] (native Pigeon path), a plain JSON map (Instant JSON
  /// on Android/iOS), or a `dartify()` map (Web). Returns `null` on anything
  /// unparseable instead of throwing — a raw `as MeasurementScale?` cast of a
  /// map crashes on Web and silently yields `null` on the native path.
  static MeasurementScale? _parseMeasurementScale(dynamic value) {
    if (value == null) return null;
    if (value is MeasurementScale) return value;
    if (value is Map) {
      try {
        return MeasurementScale.fromMap(Map<String, dynamic>.from(value));
      } catch (_) {
        return null;
      }
    }
  ...
  /// Parses a `measurementPrecision` value from either the enum `name`
  /// (native/Instant JSON) or `webName` (Web SDK) string, or an
  /// already-typed [MeasurementPrecision]. Returns `null` when unrecognized.
  static MeasurementPrecision? _parseMeasurementPrecision(dynamic value) {
    if (value == null) return null;
    if (value is MeasurementPrecision) return value;
    if (value is String) {
      for (final p in MeasurementPrecision.values) {
        if (p.name == value || p.webName == value) return p;
      }
    }
    return null;
  }
  ...
  ///
  /// Returns `null` instead of throwing when the annotation type is unknown or
  /// the payload can't be parsed, so one malformed annotation never fails a
  /// whole page's parse. This covers the platform fallbacks the deep-dive
  /// surfaced: the Android/iOS detach fallback (`pspdfkit/unknown`), Web-only
  /// types not in the model (`pspdfkit/comment`), and empty/absent types.
