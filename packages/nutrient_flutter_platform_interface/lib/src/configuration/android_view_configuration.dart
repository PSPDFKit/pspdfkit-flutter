...
class AndroidViewConfiguration {
  ...
  /// Show the stylus tool button on the annotation creation toolbar.
  ///
  /// When `false`, the button is hidden regardless of whether a stylus is
  /// detected as connected.
  ///
  /// Unlike the other options here, this isn't a `PdfActivityConfiguration`
  /// builder property — it maps to `AnnotationToolbar.setShouldShowStylusButton(boolean)`,
  /// applied on the live annotation toolbar rather than at configuration time.
  ///
  /// Because of that, it is only applied by `NutrientView`. `NutrientInstantView`
  /// takes the same configuration but builds a plain `InstantPdfUiFragment`,
  /// which has no hook to apply this on, so the option is ignored there.
