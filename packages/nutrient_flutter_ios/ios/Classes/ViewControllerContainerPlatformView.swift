...
public class ViewControllerContainerPlatformView: NSObject, FlutterPlatformView {
    ...
    /// Returns a raw pointer to the hosted UIViewController for a given view ID,
    /// or NULL if no view controller is registered for that ID.
    ///
    /// Called from Dart via FFI (`nutrient_get_view_controller`). The caller must
    /// not release the returned pointer — it is unretained.
    @objc public static func getViewController(_ viewId: Int64) -> UnsafeMutableRawPointer? {
        let lookup: () -> UnsafeMutableRawPointer? = {
            guard let container = instances.object(forKey: NSNumber(value: viewId)),
                  let vc = container.hostingController else {
                return nil
            }
    ...
    @objc public static func attachViewController(
        ...
        let attach: () -> Bool = {
            ...
            guard let flutterViewController = container.hostViewController() ?? container.findFlutterViewController() else {
                return false
            }

            flutterViewController.addChild(hostedController)
            // If the parent is already visible, manually trigger appearance
            // transitions so the child receives viewWillAppear/viewDidAppear.
            // This is required by UIKit when adding a child VC after the parent
            // has already appeared — without it PSPDFInstantViewController never
            // starts listening for server changes or shows the document.
            let parentVisible = flutterViewController.viewIfLoaded?.window != nil
            if parentVisible {
                hostedController.beginAppearanceTransition(true, animated: false)
            }
            ...
            if parentVisible {
                hostedController.endAppearanceTransition()
            }
    ...
    /// Returns the view controller that actually hosts `containerView` in the live
    /// view hierarchy, found by walking the responder chain.
    ///
    /// This is preferred over `findFlutterViewController()` (which resolves the
    /// window's root view controller) because it keeps the UIKit containment chain
    /// consistent with where `containerView` really lives. An inconsistent chain
    /// breaks trait-collection and presentation-context propagation, which surfaces
    /// when a full-screen modal presented from the embedded controller is dismissed
    /// (e.g. the camera image picker during signature/image annotation creation):
    /// on return the SwiftUI signature view re-lays out with a corrupted context and
    /// drops its navigation bar items.
